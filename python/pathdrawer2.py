...
class graphPaths():
    ...
    #####################
    def is_valid_edge_path(self,path, G):
    ...
    #####################
    def is_valid_node_path(self,path, G):
    ...
    #####################
    def to_node_path(self, edge_path):
    ...
    #####################
    def to_edge_path(self,path, G=None):
        ...
        if self.is_valid_node_path(path,G):
            return self.to_edge_path(path)
        else:
            if not self.is_valid_edge_path(path,G): raise ValueError('Not a valid path:\npath='+str(path))
            return path
    ...
    #####################
    def crossing_point(self,p1a, p1b, p2a, p2b):
    ...
    #####################
    def is_layout_normalized(self,pos):
    ...
    #####################
    def draw_path(self,G, pos, path, shifts=None, color='r', linestyle='solid', linewidth=1.0):
        ...
        if not self.is_layout_normalized(pos):   raise ValueError('Layout is not normalized!')
        edge_path = self.to_edge_path(path, G)
        ...
        edge_shifts = [ shifts[i]*self.perpendicular_vector(self.norm_vector(p1b-p1a)) for i,(p1a,p1b) in enumerate(edge_pos)]
        ...
        for i in range(len(edge_pos)-1):
            ...
            V1 = self.norm_vector(p1b - p1a)                         # unit vector along the (i)th edge
            V2 = self.norm_vector(p2b - p2a)                         # unit vector along the (i+1)th edge
            p_middle_angle = p_node + (V2-V1)                   # a point that splits evenly the angle between the original (i)th and (i+1)th edges
            c12 = self.crossing_point(p1a, p1b, p2a, p2b)            # crossing point of the shifted (i)th and (i+1)th edges
            if c12==None:   # the edges are parallel
                c12 = (p1b+p2a)/2
                p_middle_angle = c12
            c1 = self.crossing_point(p1a,p1b,p_node,p_middle_angle)  # crossing point of the shifted (i)th edge and the middle-angle-line
            c2 = self.crossing_point(p2a,p2b,p_node,p_middle_angle)  # crossing point of the shifted (i+1)th edge and the middle-angle-line
            D= 0.5*(shifts[i]+shifts[i+1])                      # average shift - a reasonable normalized distance measure

            if self.vector_length(p_node-c12) < 2.5*D:               # if the crossing point c12 is 'relatively close' to the node
        ...
        for i,corner in enumerate(corners):
            ...
            if i>0:
                if self.vector_length(self.norm_vector(corner[0]-vertices[-1]) - self.norm_vector(corner[1]-corner[0]))>1:
                    vertices.pop();
                    vertices.pop();
                    vertices += corner[-2:]
                    continue
    ...
    #####################
    def draw_many_paths(self,G, pos, paths, max_shift=0.02, linewidth=2.0):
        ...
        edge_paths=[self.to_edge_path(path,G) for path in paths]
        ...
        for i,path in enumerate(edge_paths):
            ...
            self.draw_path(G, pos, path, color=colors[i%len(colors)], linestyle=linestyles[i/len(colors) % len(linestyles)], linewidth=linewidth, shifts=shifts)
...
if __name__ == "__main__":
    ...
    networkx.draw(g,pos, node_size=140)
    pathDrawer.draw_many_paths(g, pos, [path1, path2, path3, path4], max_shift=0.03)
